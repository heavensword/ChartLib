...
@interface ISSChartHistogramStackData()

@property (nonatomic, retain) NSArray *barGroups;

@end

@implementation ISSChartHistogramStackData
...
- (id)initWithHistogramStackData:(ISSChartHistogramStackData*)histogramOverlapData
{
    self = [super init];
    if (self) {
...
{
    ISSChartHistogramStackData *copy = [[ISSChartHistogramStackData allocWithZone:zone] initWithHistogramStackData:self];
    return copy;
}
...
- (void)setBarDataArraysWithValues:(NSArray*)values1, ...NS_REQUIRES_NIL_TERMINATION
{
    ...
    [self setBarValues:valuesArrays];
}
    ...
    if (!_legends) {
        ...
        if (legendCount) {
            ...
            for (NSInteger i = 0; i < lineCount && index < legendCount; i++) {
                ...
                ISSChartLegend *legend = [[ISSChartLegend alloc] init];
                legend.name = self.legendArray[index];
